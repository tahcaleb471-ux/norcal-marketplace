...
import { defaultCars, getCars, getCurrentUser, getUsers, saveUserProfile, type User } from '@/lib/norcal-data';
...
export default function ProfilePage({ params }: { params: { userId: string } }) {
  ...
  useEffect(() => {
    ...
    const found = users.find((item) => item.id === params.userId) || users[0];
    setProfile(found);
    setCars(getCars().filter((car) => car.sellerId === found.id));
  }, [params.userId]);
        ...
        <div className="card rounded-[32px] p-6 md:p-8">
          <div className="flex flex-col md:flex-row items-start gap-6">
            ...
            <div className="flex-1">
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                ...
                <Link href="/messages" className="rounded-full bg-green-600 px-5 py-2.5 font-bold text-white hover:bg-green-500">Message seller</Link>
              </div>
          ...
          <div className="mt-8 grid md:grid-cols-3 gap-4">
            ...
            <div className="rounded-2xl bg-slate-50 p-4">
              ...
              <div className="mt-2 text-3xl font-black">{cars.length}</div>
            </div>
            <div className="rounded-2xl bg-slate-50 p-4">
